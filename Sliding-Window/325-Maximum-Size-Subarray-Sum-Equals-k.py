...
class Solution(object):
    def findMaxSumSubArray(self, k, nums):
        ...
        for end in range(len(nums)):
            curr_sum += nums[end]
            # Chỉ slide window khi vượt limit size k. And also chỉ update max khi này phòng TH max bị update khi chưa đủ
            # required len. Ta adjust lại 1 đơn vị để end luôn = k - 1.
            if end >= k - 1:
